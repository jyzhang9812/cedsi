<template>
  <el-container>
    <el-aside width="220px">
      <el-menu default-active="0" class="el-menu-vertical-demo" background-color="#3766A6" text-color="#fff"
        active-text-color="#ffd04b" @select="handleSelect">
        <el-submenu v-for="(item, index) in submenu" :key="index" :index="item.name">
          <template slot="title">
            <i :class="item.icon"></i>
            <span>{{item.name}}</span>
          </template>
