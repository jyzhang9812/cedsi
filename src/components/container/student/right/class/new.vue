  ...
  <div id="new" class="limiter">
    ...
    <div class="container-table100" v-else>
      <div style="margin-bottom: 10px;">
        <button class="nameOfClass" v-for="(item,index) in myClass" @click="currentId = index" :key="index"
          :class="{active : index===currentId}">{{item.name}}</button>
      </div>
      <div class="wrap-table100">
        <div style="margin-bottom: 10px;">
          <img class="avastimg" :src="nowClass.teacher.avatar" />
          <span>代课老师：{{nowClass.teacher.teacher_name}}</span>
          <span style="margin-left: 20px">班级人数：{{nowClass.member_count}}</span>
        </div>
          ...
          <table data-vertable="ver1">
            ...
            <tbody>
              <tr class="row100" v-for="(item,index) in nowClass.classmates" :key="index">
                ...
                <td class="column100">
                  <img class="avastimg" :src="item.STUDENT_INFO.AVATAR" />
                </td>
                <td class="column100">{{item.STUDENT_INFO.STUDENT_NAME}}</td>
                <td class="column100">{{item.STUDENT_INFO.GENDER}}</td>
                <td class="column100">{{item.STUDENT_INFO.AGE}}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      nowClass(){
        return this.myClass[this.currentId]
      }
      // nowClass: () => this.myClass[this.currentId]
    },
    created: function () {
      ...
      this.$store.dispatch("getClass").then(() => {
        if (this.myClass.length != 0) {
          this.txt = false;
          // this.nowClass = this.myClass[this.currentId];
        }
      });
      // this.nowClass = this.myClass[0];
    },
    // computed: {
    //   ...mapState({
    //     myClass: state => state.myClass
