<template>
  <div id="choiceness">
    ...
    <div class="tab-bar">
      <span
        v-for="(title, index) in tabBarTitle"
        :key="index"
        @click="tabBarChange(title)"
      >{{title}}</span>
    </div>
    <div class="divider">
    ...
    <div class="panels" v-if="currentTabBarTitle === tabBarTitle[0]">
      ...
      <pagination :num="tableData_0.length" @getNew="changeFirstTablePages" :limit="limit"></pagination>
    </div>
    <div class="uploadWorks" v-if="currentTabBarTitle === tabBarTitle[1]">
      <div v-show="!isUploadWorks">
        ...
        <pagination :num="tableData_1.length" @getNew="changeSecondTablePages" :limit="limit"></pagination>
      </div>
      <div v-show="isUploadWorks" class="uploadWorks-page">
        <div>
          <span>选择文件</span>
          <button>选择作品</button>
          <label>liuxingyu.sb3</label>
        </div>
        <div>
          <span>作者</span>
        ...
        <div>
          ...
          <div style="display: inline">
            <select-input
              id="uploadWorks_school"
              :tips="uploadWorks.tips"
              :dropDownList="uploadWorks.school"
        ...
        <div>
          <span>操作说明</span>
          <textarea
            placeholder="如果你的作品需要操作互动,请不要忘记告诉大家操作方法哦!"
            v-model="uploadWorks.operatingInstruction"
          ></textarea>
        </div>
        <div>
          <span>上传封面</span>
...
export default {
  ...
  data() {
    return {
      ...
      tableData_1: [
        ["赛小迪", "2019-5-2 19:30", "超级玛丽", "师大附小"],
        ["赛大迪", "2017-5-2 15:03", "超级玛丽", "师大附小"]
      ],
      tabBarStyle: {
        width: "",
        "margin-left": ""
  ...
  methods: {
    ...
    changeSecondTablePages(value) {
      this.currentSecondList = this.tableData_1.slice(
        value,
        value + this.limit
      );
    },
