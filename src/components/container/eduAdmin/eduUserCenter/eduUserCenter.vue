 ...
 * @Date: 2019-11-06 22:20:37
 * @Description: 教务角色个人中心页面
 -->
<template>
    ...
    <div class="contentdetail">
      <form class="form" @submit.prevent="submit">
        ...
        <div class="formdetail">
          ...
          <input class="input" placeholder="请输入固话号码" v-model="user.phone" />
        </div>
        <div class="formdetail">
        ...
        <div class="formdetail" v-show="updatePW == false">
          ...
          <input
            ...
            placeholder="请输入新密码"
          />
          <input class="input" type="text" v-model="newpassword" v-else placeholder="请输入新密码" />
          <span class="blue-text" @click="pwdType = !pwdType" v-if="pwdType">显示</span>
          <span class="blue-text" @click="pwdType = !pwdType" v-else>隐藏</span>
          <span class="blue-text" @click="submitPassword()" v-show="newpassword !== ''">确认修改</span>
        </div>
        <div class="formdetail">
          <button class="buttonsave" @click="submit($event)">保存</button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      headsculpture: "",
      radiolist: [
        { name: "男", value: 1, isCheck: false },
        { name: "女", value: 2, isCheck: false }
      ]
    };
  },
  methods: {
